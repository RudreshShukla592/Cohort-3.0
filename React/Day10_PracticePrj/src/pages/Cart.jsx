      ...
      {cartItems.length === 0 ? (
        ...
        <div className="flex flex-col gap-5">
          {cartItems.map((item) => (
            <div
              key={item.id}
              className="bg-gray-900 rounded-xl p-4 flex flex-col lg:flex-row lg:items-center  justify-between gap-6  border border-gray-700 hover:border-indigo-500 transition"
            >
              {/* Left */}
              <div className="flex flex-col sm:flex-row items-center gap-5 flex-1">
                <div className="bg-white rounded-lg p-2 w-24 h-24 sm:w-28 sm:h-28 flex justify-center items-center">
                  <img
                    src={item.image}
                    alt={item.title}
                ...
                <div>
                  <h2 className="font-semibold text-lg max-w-full lg:max-w-md line-clamp-2">
                    {item.title}
                  </h2>
              ...
              <div className="flex flex-col items-center lg:items-end gap-3">
                ...
                <button onClick={() => removeFromCart(item.id)} className="w-full lg:w-36 bg-red-500 hover:bg-red-600 py-2 rounded-lg transition">
                  Remove
                </button>
              </div>
            </div>
